...
export default function Auth() {
  ...
  const SubmitBtn = ({ label }: { label: string }) => (
    <button
      type="submit"
      disabled={isLoading}
      className="w-full py-4 bg-gradient-to-r from-yellow-400 to-yellow-500 rounded-xl text-black font-headline font-bold shadow-lg shadow-yellow-400/20 hover:shadow-yellow-400/40 hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:translate-y-0 relative overflow-hidden flex items-center justify-center gap-2"
    >
      {isLoading
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden bg-surface-container-lowest">
      <div className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] rounded-full bg-yellow-400/5 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-20%] right-[-10%] w-[400px] h-[400px] rounded-full bg-yellow-400/3 blur-[100px] pointer-events-none" />
      ...
      >
        ...
        >
          ...
          <span className="font-headline text-sm uppercase tracking-widest font-bold">Back</span>
        </Link>
      </motion.div>
      ...
      >
        <div className="w-10 h-10 rounded-xl bg-yellow-400 flex items-center justify-center shadow-lg shadow-yellow-400/30">
          <span className="text-black font-headline text-xl font-black">F</span>
        </div>
        <h1 className="text-2xl font-headline tracking-tighter text-white">FlexProtocol</h1>
      </motion.div>

      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="w-full max-w-[420px] bg-surface-container/80 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl relative z-10 overflow-hidden"
      >
        <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-yellow-400/50 to-transparent" />

        <div className="p-8">
          <div className="relative flex p-1 bg-black/30 rounded-2xl mb-8 border border-white/5">
            <motion.div
              className="absolute top-1 bottom-1 left-1 bg-yellow-400/15 border border-yellow-400/30 rounded-xl z-0"
              initial={false}
            ...
            {(['signin', 'signup'] as const).map(tab => (
              <button
                ...
                className={`relative z-10 flex-1 py-2.5 text-sm font-headline font-bold uppercase tracking-widest transition-colors duration-300 ${
                  activeTab === tab ? 'text-yellow-400' : 'text-white/40'
                }`}
              >
            ...
            {activeTab === 'signin' ? (
              ...
              >
                <div className="text-center">
                  <h2 className="text-2xl font-headline text-white mb-1">Welcome back</h2>
                ...
                <form className="space-y-4" onSubmit={handleLogin}>
                  ...
                  <div className="text-right">
                    <button
                      ...
                      className="text-[10px] uppercase tracking-widest text-yellow-400 font-bold hover:text-yellow-300 transition-colors"
                    >
                      Forgot password?
                    </button>
                  </div>
              ...
              >
                <div className="text-center">
                  <h2 className="text-2xl font-headline text-white mb-1">Create account</h2>
                ...
                <form className="space-y-4" onSubmit={handleSignup}>
                  ...
                  <p className="text-[10px] text-white/30 text-center leading-relaxed">
                    ...
                    <Link to="/terms-and-conditions" className="text-yellow-400 hover:underline">Terms</Link>
                    {' & '}
